export function initForm() {
  ...
  if (direccion) {
    direccion.addEventListener('input', () => {
      if (direccion.value.length > maxCaracteres)
        direccion.value = direccion.value.slice(0, maxCaracteres)
  ...
  if (contrasena && repiteContrasena) {
    contrasena.addEventListener('input', () => {
      ...
      if (
        repiteContrasena.value.trim() &&
        contrasena.value === repiteContrasena.value
      )
        errores.repiteContrasena.textContent = ''
    })
