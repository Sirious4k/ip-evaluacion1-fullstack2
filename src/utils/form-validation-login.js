export function initFormLogin() {
  ...
  if (logCorreo) {
    logCorreo.addEventListener('input', () => {
      if (/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(logCorreo.value.trim()))
        logErrores.logCorreo.textContent = ''
    })
  }

  if (logContrasena) {
    logContrasena.addEventListener('input', () => {
      if (logContrasena.value.trim().length >= 8)
        logErrores.logContrasena.textContent = ''
    })
  }
    ...
    logForm.addEventListener('submit', n => {
      ...
      if (isValid) {
        requestAnimationFrame(() => {
          Object.values(logErrores).forEach(span => {
