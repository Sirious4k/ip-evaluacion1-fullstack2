...
function Sidebar({ categories = [], selected }) {
  const styles = {
    ...
    linkDefault:
      'flex format-text-p  !not-italic !text-white px-5 py-2 hover:bg-gray-700 !text-2xl hover:!bg-[var(--bg-button)] ',
    linkSelected:
      'flex format-text-p !not-italic !text-white !opacity-100 bg-[var(--bg-button)] px-5 py-2  !text-2xl ',
  }
        ...
        {categories.map(cat => (
          <li key={cat}>
            <Link
              ...
              className={
                selected === cat ? styles.linkSelected : styles.linkDefault
              }
            >
              {cat}
            </Link>
